...
// Keep this connector local so dev builds do not compile every optional wagmi wallet connector.
type Mutable<T> = { -readonly [P in keyof T]: T[P] }

export type BaseAccountParameters = Mutable<
  Omit<Parameters<typeof createBaseAccountSDK>[0], 'appChainIds'>
>
...
export function baseAccount(parameters: BaseAccountParameters = {}) {
  ...
  return createConnector<Provider, BaseAccountProperties>((config) => ({
    id: 'baseAccount',
    name: 'Base Account',
    ...
    async connect({ chainId, withCapabilities, ...rest } = {}) {
      try {
        ...
        const connection = await (async () => {
          if (rest.isReconnecting) {
            return {
              ...
              ).map((address) => ({ address: getAddress(address) })),
          ...
          const accounts = orderedAccounts.map(
            (orderedAddress) =>
              response.accounts.find(
                (account) => account.address === orderedAddress,
        ...
        let currentChainId = connection.currentChainId
        ...
        if (chainId && currentChainId !== chainId) {
          const chain = await this.switchChain!({ chainId }).catch((error) => {
            if ((error as { code?: number }).code === UserRejectedRequestError.code) {
              throw error
            }
