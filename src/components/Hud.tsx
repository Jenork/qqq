      ...
      <div className="pointer-events-auto flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-full border border-white/10 bg-black/28 px-3 py-2 text-[10px] font-black uppercase tracking-[0.15em] text-stone-100 backdrop-blur"
          >
            Inv
          </button>
          <button
        ...
        <div className="w-[min(100%,430px)] rounded-full border border-white/10 bg-black/28 px-3 py-2.5 backdrop-blur">
          <div className="flex items-center gap-3">
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex items-center justify-between gap-2">
                <p className="truncate text-[10px] font-black uppercase tracking-[0.14em] text-orange-200">
                  HP {Math.ceil(hp)}/{maxHp}
                </p>
                <div className="flex items-center gap-1.5">
                  <span className="rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-orange-100">
                    W {wave}
                  </span>
                  <span className="rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-amber-200">
                    S {score}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-orange-100">
                  {grenadeIcon ? (
                    <img src={grenadeIcon} alt="" className="h-4 w-4 object-contain [image-rendering:pixelated]" />
                  ) : null}
                  GR {grenadeLabel}
                </span>
                <span
                  className={cn(
                    'flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em]',
                    shieldRemaining > 0
                      ? 'border-cyan-300/25 bg-cyan-500/12 text-cyan-100'
                      : 'border-white/10 bg-black/30 text-orange-100',
                  )}
                >
                  {abilityIcon ? (
                    <img src={abilityIcon} alt="" className="h-4 w-4 object-contain [image-rendering:pixelated]" />
                  ) : null}
                  AB {abilityLabel}
                </span>
                <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-orange-100">
                  {healIcon ? (
                    <img src={healIcon} alt="" className="h-4 w-4 object-contain [image-rendering:pixelated]" />
                  ) : null}
                  HL {healLabel}
                </span>
              </div>
          ...
          {activeMessage ? (
            <p className="mt-1 truncate text-[9px] uppercase tracking-[0.14em] text-orange-100/75">{activeMessage}</p>
          ) : null}
        </div>
      </div>
